...
// Khai báo API URL chính xác mà bạn đã cung cấp
...
class UltraDicePredictionSystem {
    ...
    // Phương thức đã được sửa để đặt trong lớp
    addResult(result) {
    ...
    // Phương thức cốt lõi đã được thêm vào
    getAllPredictions() {
        const predictions = {};
    ...
    // Phương thức đã được sửa lỗi cú pháp
    ...
    // ===============================================
    // PHƯƠNG THỨC HỖ TRỢ CẦN THIẾT (Đã được bổ sung)
    // ===============================================
    ...
    updatePerformance(result) {
        for (const modelName of Object.keys(this.performance)) {
            if (modelName.match(/^model\d+$/)) {
                ...
                // Cần cache dự đoán của vòng trước nếu muốn tính toán chính xác
                // Tuy nhiên, theo logic hiện tại, ta chấp nhận gọi lại model để xem nó dự đoán gì dựa trên lịch sử CŨ
    ...
    getFinalPrediction() {
        // Chạy model 7 để điều chỉnh trọng số trước
        this.model7();
...
app.get('/predict', async (req, res) => {
    try {
        ...
        if (axios.isAxiosError(error)) {
            if (error.response) {
                // Lỗi HTTP Status Code (ví dụ: 404, 500)
                ...
                // Lỗi kết nối mạng (ví dụ: Timeout, DNS error)
